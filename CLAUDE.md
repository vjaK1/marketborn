...
- **Time**: 1 tick = 1 simulated day. Every tick executes a fixed, documented phase order: apply queued commands → scheduled events → production → labor market → goods markets → contract settlement → banking → government → consumption → agent decisions → memory/relationship updates → metrics, invariant checks, hashing. The phase order is part of the determinism contract and lives in `docs/ECONOMIC_RULES.md` (government slot added by DECISIONS.md #029).
